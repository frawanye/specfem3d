...
#ifdef USE_TEXTURES_FIELDS
  __builtin_nontemporal_store(texfetch_displ<FORWARD_OR_ADJOINT>((*iglob)*3), &sh_displx[(*tx)]);
  __builtin_nontemporal_store(texfetch_displ<FORWARD_OR_ADJOINT>((*iglob)*3 + 1), &sh_disply[(*tx)]);
  __builtin_nontemporal_store(texfetch_displ<FORWARD_OR_ADJOINT>((*iglob)*3 + 2), &sh_displz[(*tx)]);
#else
  // changing iglob indexing to match fortran row changes fast style
  const realw x = __builtin_nontemporal_load(&d_displ[(*iglob)*3]);
  const realw y = __builtin_nontemporal_load(&d_displ[(*iglob)*3] + 1);
  const realw z = __builtin_nontemporal_load(&d_displ[(*iglob)*3] + 2);
  __builtin_nontemporal_store(x, &sh_displx[(*tx)]);
  __builtin_nontemporal_store(y, &sh_disply[(*tx)]);
  __builtin_nontemporal_store(z, &sh_displz[(*tx)]);
#endif
...
#ifdef USE_TEXTURES_CONSTANTS
  // hprime
  __builtin_nontemporal_store(tex1Dfetch(d_hprime_xx_tex,tx + d_hprime_xx_tex_offset), &sh_hprime_xx[(*tx)]);
#else
  // hprime
  const realw temp = __builtin_nontemporal_load(&d_hprime_xx[(*tx)]);
  __builtin_nontemporal_store(temp, &sh_hprime_xx[(*tx)]);
#endif
}
...
__device__  __forceinline__ void load_shared_memory_hprimewgll(const int* tx,
  ...
  // each thread reads its corresponding value
  // weighted hprime
  const realw temp = __builtin_nontemporal_load(&d_hprimewgll_xx[(*tx)]);
  __builtin_nontemporal_store(temp, &sh_hprimewgll_xx[(*tx)]);
}
...
__device__  __forceinline__ void sum_hprime_xi(int I, int J, int K,
  ...
  for (int l=0;l<NGLLX;l++) {
    fac = __builtin_nontemporal_load(&sh_hprime[l*NGLLX+I]);

    sumx += __builtin_nontemporal_load(&sh_tempx[K*NGLL2+J*NGLLX+l]) * fac;
    sumy += __builtin_nontemporal_load(&sh_tempy[K*NGLL2+J*NGLLX+l]) * fac;
    sumz += __builtin_nontemporal_load(&sh_tempz[K*NGLL2+J*NGLLX+l]) * fac;
  }
...
__device__  __forceinline__ void sum_hprime_eta(int I, int J, int K,
  ...
  #pragma unroll
  for (int l=0;l<NGLLX;l++) {
    fac = __builtin_nontemporal_load(&sh_hprime[l*NGLLX+J]);

    sumx += __builtin_nontemporal_load(&sh_tempx[K*NGLL2+l*NGLLX+I]) * fac;
    sumy += __builtin_nontemporal_load(&sh_tempy[K*NGLL2+l*NGLLX+I]) * fac;
    sumz += __builtin_nontemporal_load(&sh_tempz[K*NGLL2+l*NGLLX+I]) * fac;
  }
...
__device__  __forceinline__ void sum_hprime_gamma(int I, int J, int K,
  ...
  #pragma unroll
  for (int l=0;l<NGLLX;l++) {
    fac = __builtin_nontemporal_load(&sh_hprime[l*NGLLX+K]);

    sumx += __builtin_nontemporal_load(&sh_tempx[l*NGLL2+J*NGLLX+I]) * fac;
    sumy += __builtin_nontemporal_load(&sh_tempy[l*NGLL2+J*NGLLX+I]) * fac;
    sumz += __builtin_nontemporal_load(&sh_tempz[l*NGLL2+J*NGLLX+I]) * fac;
  }
...
__device__  __forceinline__ void sum_hprimewgll_xi(int I, int J, int K,
  ...
  #pragma unroll
  for (int l=0;l<NGLLX;l++) {
    fac = __builtin_nontemporal_load(&sh_hprimewgll[I*NGLLX+l]); //  d_hprimewgll_xx[I*NGLLX+l];
...
__device__  __forceinline__ void sum_hprimewgll_eta(int I, int J, int K,
  ...
  #pragma unroll
  for (int l=0;l<NGLLX;l++) {
    fac = __builtin_nontemporal_load(&sh_hprimewgll[J*NGLLX+l]); // d_hprimewgll_xx[J*NGLLX+l];
...
__device__  __forceinline__ void sum_hprimewgll_gamma(int I, int J, int K,
  ...
  #pragma unroll
  for (int l=0;l<NGLLX;l++) {
    fac = __builtin_nontemporal_load(&sh_hprimewgll[K*NGLLX+l]); // d_hprimewgll_xx[K*NGLLX+l];
...
__device__  __forceinline__ void get_dot_product(realw jacobianl,
  ...
  if (threadIdx.x < NGLL3) {
    if (ispec_irreg >= 0){
      //irregular element
      __builtin_nontemporal_store(jacobianl * (sigma_xx*Dxl + sigma_yx*Dyl + sigma_zx*Dzl), &sh_tempx[tx]); // sh_tempx1
      __builtin_nontemporal_store(jacobianl * (sigma_xy*Dxl + sigma_yy*Dyl + sigma_zy*Dzl), &sh_tempy[tx]); // sh_tempy1
      __builtin_nontemporal_store(jacobianl * (sigma_xz*Dxl + sigma_yz*Dyl + sigma_zz*Dzl), &sh_tempz[tx]); // sh_tempz1
    }
    else if (component == 1){
      __builtin_nontemporal_store(jacobian_regular * (sigma_xx*xix_regular), &sh_tempx[tx]); // sh_tempx1
      __builtin_nontemporal_store(jacobian_regular * (sigma_xy*xix_regular), &sh_tempy[tx]); // sh_tempy1
      __builtin_nontemporal_store(jacobian_regular * (sigma_xz*xix_regular), &sh_tempz[tx]); // sh_tempz1
    }
    else if (component == 2){
      __builtin_nontemporal_store(jacobian_regular * (sigma_yx*xix_regular), &sh_tempx[tx]); // sh_tempx1
      __builtin_nontemporal_store(jacobian_regular * (sigma_yy*xix_regular), &sh_tempy[tx]); // sh_tempy1
      __builtin_nontemporal_store(jacobian_regular * (sigma_yz*xix_regular), &sh_tempz[tx]); // sh_tempz1

    }else{
      __builtin_nontemporal_store(jacobian_regular * (sigma_zx*xix_regular), &sh_tempx[tx]); // sh_tempx1
      __builtin_nontemporal_store(jacobian_regular * (sigma_zy*xix_regular), &sh_tempy[tx]); // sh_tempy1
      __builtin_nontemporal_store(jacobian_regular * (sigma_zz*xix_regular), &sh_tempz[tx]); // sh_tempz1
    }
  }
  __syncthreads();
}
...
// + 0 BYTE
  ...
  #else
  realw* sh_hprime_xx     = (realw*) &d_hprime_xx[0];
  realw* sh_hprimewgll_xx = (realw*) &d_hprimewgll_xx[0];  
  #endif
