...
__global__ void compute_stacey_elastic_single_kernel(realw* veloc,
  ...
  //if (igll < NGLL2 && iface < num_abs_boundary_faces) {
    ...
    // "-1" from index values to convert from Fortran-> C indexing
    ispec = __builtin_nontemporal_load(&abs_boundary_ispec[iface])-1;

    if (ispec_is_elastic[ispec]) {

      i = __builtin_nontemporal_load(&abs_boundary_ijk[INDEX3(NDIM,NGLL2,0,igll,iface)])-1;
      j = __builtin_nontemporal_load(&abs_boundary_ijk[INDEX3(NDIM,NGLL2,1,igll,iface)])-1;
      k = __builtin_nontemporal_load(&abs_boundary_ijk[INDEX3(NDIM,NGLL2,2,igll,iface)])-1;

      iglob = __builtin_nontemporal_load(&d_ibool[INDEX4_PADDED(NGLLX,NGLLX,NGLLX,i,j,k,ispec)])-1;
      ...
      if (FORWARD_OR_ADJOINT == 3){
        ...
        // gets associated velocity
        vx = __builtin_nontemporal_load(&veloc[iglob*3+0]);
        vy = __builtin_nontemporal_load(&veloc[iglob*3+1]);
        vz = __builtin_nontemporal_load(&veloc[iglob*3+2]);

        // gets associated normal
        nx = __builtin_nontemporal_load(&abs_boundary_normal[INDEX3(NDIM,NGLL2,0,igll,iface)]);
        ny = __builtin_nontemporal_load(&abs_boundary_normal[INDEX3(NDIM,NGLL2,1,igll,iface)]);
        nz = __builtin_nontemporal_load(&abs_boundary_normal[INDEX3(NDIM,NGLL2,2,igll,iface)]);

        // // velocity component in normal direction (normal points out of element)
        vn = vx*nx + vy*ny + vz*nz;

        rho_vp_temp = __builtin_nontemporal_load(&rho_vp[INDEX4(NGLLX,NGLLX,NGLLX,i,j,k,ispec)]);
        rho_vs_temp = __builtin_nontemporal_load(&rho_vs[INDEX4(NGLLX,NGLLX,NGLLX,i,j,k,ispec)]);

        tx = rho_vp_temp*vn*nx + rho_vs_temp*(vx-vn*nx);
        ty = rho_vp_temp*vn*ny + rho_vs_temp*(vy-vn*ny);
        tz = rho_vp_temp*vn*nz + rho_vs_temp*(vz-vn*nz);

        jacobianw = __builtin_nontemporal_load(&abs_boundary_jacobian2Dw[INDEX2(NGLL2,igll,iface)]);
        ...
        if (SAVE_FORWARD && SIMULATION_TYPE == 1) {
          __builtin_nontemporal_store(tx*jacobianw, &b_absorb_field[INDEX3(NDIM,NGLL2,0,igll,iface)]);
          __builtin_nontemporal_store(ty*jacobianw, &b_absorb_field[INDEX3(NDIM,NGLL2,1,igll,iface)]);
          __builtin_nontemporal_store(tz*jacobianw, &b_absorb_field[INDEX3(NDIM,NGLL2,2,igll,iface)]);
        } // SIMULATION_TYPE
      }
    }
  } // num_abs_boundary_faces
